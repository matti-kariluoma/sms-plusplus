...
def process_sms_command(user, command):
	...
	if cmd.startswith('prev'):
		...
		error = True
		try:
			webpage = render_url(str(command).split(' ')[1])
			error = False
		except IndexError:
			pass
		...
		error = True
		try:
			webpage = render_url('http://134.129.125.232:8080/smsplusplus/query.action;service=mail&phone=%s' % (phonenumber))
			print len(webpage)
		...
		error = True
		try:
			webpage = render_url('http://134.129.125.232/gcalendar.php?number=%s&mode=json' % (phonenumber))
			lines = webpage.splitlines()
			for line in lines:
				line.strip()
			webpage = ''.join(lines)
